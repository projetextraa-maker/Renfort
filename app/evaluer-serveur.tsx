...
export default function EvaluerServeur() {
  ...
  const handleEvaluer = async () => {
    if (note === 0) { Alert.alert('Erreur', 'Veuillez choisir une note'); return }
    setLoading(true)
    try {
      const {
        data: { user },
      ...
      if (!user || typeof annonceId !== 'string' || typeof serveurId !== 'string') {
        Alert.alert('Erreur', 'Impossible d enregistrer cette evaluation.')
        return
      }
      ...
      Alert.alert('Merci !', 'Votre evaluation a ete enregistree.', [
  ...
  return (
    <ScrollView style={s.screen} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      ...
      <Text style={s.title}>Evaluer le prestataire</Text>
      <Text style={s.subtitle}>Comment s&apos;est passee la mission avec {serveurNom} ?</Text>
      ...
      <View style={s.criteresCard}>
        <Text style={s.criteresTitle}>Ce que vous evaluez</Text>
        {['Ponctualite', 'Qualite du service', 'Professionnalisme', 'Communication'].map(c => (
      ...
      <View style={s.commentCard}>
        <Text style={s.commentLabel}>Commentaire (optionnel)</Text>
        <TextInput
...
const s = StyleSheet.create({
  ...
  criteresCard:  { backgroundColor: C.card, borderRadius: 18, borderWidth: 1, borderColor: C.border, padding: 18, marginBottom: 14, shadowColor: '#000', shadowOpacity: 0.04, shadowOffset: { width: 0, height: 1 }, shadowRadius: 4, elevation: 1 },
  ...
  commentCard:  { backgroundColor: C.card, borderRadius: 18, borderWidth: 1, borderColor: C.border, padding: 18, marginBottom: 24, shadowColor: '#000', shadowOpacity: 0.04, shadowOffset: { width: 0, height: 1 }, shadowRadius: 4, elevation: 1 },
  commentLabel: { fontSize: 13, fontWeight: '700', color: C.title, marginBottom: 10 },
  commentInput: { height: 90, fontSize: 14, color: C.title, lineHeight: 20 },

  btn:        { backgroundColor: C.terra, borderRadius: 16, paddingVertical: 16, alignItems: 'center', marginBottom: 12, shadowColor: C.terra, shadowOpacity: 0.2, shadowOffset: { width: 0, height: 4 }, shadowRadius: 10, elevation: 2 },
